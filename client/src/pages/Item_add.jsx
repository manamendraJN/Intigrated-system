...
export default function Item_add() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      setError(false); // Setting the error status to null after sending the data
      const res = await fetch("/api/Item", {
        method: 'post',
        headers: { 'Content-Type': 'application/json' },
        // Adding user reference to the item data
        body: JSON.stringify({ ...addItemData, userRef: currentUser._id })
      ...
      if (data.success === false) {
        setError(true);
        return;
      } // Setting the error status to true if the data is not sent
    } catch (error) {
      setLoading(false); // Setting the loading status to false after error
      setError(true); // Setting the error status to error after error
    }
